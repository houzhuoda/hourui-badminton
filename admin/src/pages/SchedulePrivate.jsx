...
import { Card, Select, DatePicker, Button, Space, Row, Col, Tag, Tabs, Alert } from 'antd';
...
export default function SchedulePrivate() {
  ...
  const toggleSlot = async (slot) => {
    ...
    try {
      await api.put(`/private-bookings/${selectedCoach}/toggle-slot`, {
        date: gridDate.format('YYYY-MM-DD'),
        startHour: slot.hour,
        status: newStatus,
      });
      loadGrid();
  ...
  const batchSetAll = async (status) => {
    for (const s of slots) {
      if (s.status === 'BOOKED' || s.status === 'CONFLICT') continue;
      if (s.status === 'UNSCHEDULED' && status === 'AVAILABLE') continue;
  ...
  const submitBatch = async (status) => {
    if (selectedDates.length === 0) { message.warning('请选择日期'); return; }
    if (selectedHours.length === 0) { message.warning('请选择时段'); return; }
    try {
      const res = await api.post(`/private-bookings/${selectedCoach}/batch-schedule`, {
        dates: selectedDates, hours: selectedHours, status,
      });
      message.success(`已批量设置 ${res.count} 个时段`);
  ...
  return (
    <div>
      <div className="flex-between mb-16">
        <h2>私教/陪练排班</h2>
        <Button type={batchMode ? 'primary' : 'default'} onClick={() => setBatchMode(!batchMode)}>
          {batchMode ? '返回单日模式' : '批量排班'}
        </Button>
      </div>

      <Alert type="info" showIcon message="选择教练和日期，在网格中点击时段切换「上班/休息」。绿色=可预约，红色=休息，蓝色=已约，橙色=有其他课程冲突。" className="mb-16" />
      ...
      {!batchMode ? (
        <>
          <Card className="mb-16">
            <Space>
              ...
              <span>日期：</span>
              <DatePicker value={gridDate} onChange={(v) => v && setGridDate(v)} />
              <Button onClick={() => batchSetAll('AVAILABLE')}>全部上班</Button>
              <Button onClick={() => batchSetAll('REST')}>全部休息</Button>
            </Space>
          </Card>
          ...
          <Row gutter={[12, 12]}>
            {HOUR_RANGE.map((h) => {
              ...
              const clickable = slot.status === 'AVAILABLE' || slot.status === 'REST';
              return (
                <Col key={h} span={4}>
                  <div onClick={() => clickable && toggleSlot(slot)}
                    style={{
        ...
        <Card>
          ...
          <Space>
            <Button type="primary" onClick={() => submitBatch('AVAILABLE')}>
              批量上班（{selectedDates.length}天 × {selectedHours.length}时段）
            </Button>
            <Button onClick={() => submitBatch('REST')}>批量休息</Button>
          </Space>
        </Card>
      )}
    </div>
