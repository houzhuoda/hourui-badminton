...
router.get('/dashboard', authRole(['sales', 'coach']), (req, res, next) => {
  try {
    ...
    const todayStats = db.prepare(`
      SELECT COUNT(*) as order_count, COALESCE(SUM(amount),0) as total_amount, COALESCE(SUM(commission_amount),0) as commission
...
router.get('/performance', authRole(['sales', 'coach']), (req, res, next) => {
  try {
    ...
    const list = db.prepare(`
      SELECT o.id, o.order_no, o.business_type, o.charge_mode, o.amount, o.commission_amount, o.commission_type, o.created_at,
    ...
    const summary = {
      ...
      totalCommission: list.reduce((s, o) => s + o.commission_amount, 0),
