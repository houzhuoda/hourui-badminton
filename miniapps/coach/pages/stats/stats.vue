...
export default {
  ...
  methods: {
    ...
    async loadCoach() {
      try {
        const user = JSON.parse(uni.getStorageSync('coach_user') || '{}');
    ...
    goCreateMember() { uni.navigateTo({ url: '/pages/sales/create-member' }); },
    goCreateOrder() { uni.navigateTo({ url: '/pages/sales/create-order' }); },
  },
};
</script>
