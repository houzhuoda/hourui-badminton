<template>
  <view class="dashboard">
    ...
    <view class="stats-grid">
      <view class="stat-card" @click="goTo('/pages/members/members')">
        <text class="stat-value">{{ stats.memberCount || 0 }}</text>
        <text class="stat-label">我的会员</text>
      </view>
      <view class="stat-card" @click="goTo('/pages/orders/list')">
        <text class="stat-value">{{ stats.orderCount || 0 }}</text>
        <text class="stat-label">本月订单</text>
      </view>
      <view class="stat-card" @click="goTo('/pages/performance/performance')">
    ...
    <view class="actions">
      <view class="action-item" @click="goTo('/pages/members/create')">
        <text class="action-icon">👤</text>
        <text class="action-text">新建会员</text>
      </view>
      <view class="action-item" @click="goTo('/pages/orders/create')">
        <text class="action-icon">📝</text>
        <text class="action-text">购课开单</text>
      </view>
      <view class="action-item" @click="goTo('/pages/members/members')">
        <text class="action-icon">📋</text>
        <text class="action-text">会员列表</text>
      </view>
      <view class="action-item" @click="goTo('/pages/orders/list')">
        <text class="action-icon">💰</text>
...
export default {
  ...
  onShow() {
    this.user = JSON.parse(uni.getStorageSync('sales_user') || '{}');
    this.loadData();
  },
  methods: {
    async loadData() {
