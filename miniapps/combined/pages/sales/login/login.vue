...
export default {
  ...
  methods: {
    async handleLogin() {
      ...
      this.loading = true;
      try {
        const data = await api.login('sales', { phone: this.phone, password: this.password });
        uni.setStorageSync('token', data.token);
        uni.setStorageSync('user', JSON.stringify(data.user));
        uni.setStorageSync('role', 'sales');
        const app = getApp();
        if (app && app.globalData) { app.globalData.token = data.token; app.globalData.user = data.user; app.globalData.role = 'sales'; }
        uni.reLaunch({ url: '/pages/sales/dashboard/dashboard' });
      } catch (e) {
        // toast 已在 api 层处理
      }
      this.loading = false;
